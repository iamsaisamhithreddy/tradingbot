...
// --- 🔴 MODIFIED SECTION: Fetch ONLY Today's Events --- 

$now = new DateTime("now", new DateTimeZone('Asia/Kolkata'));
$todayDate = $now->format('Y-m-d');

// Filter the query to only select rows matching today's date
$sql = "SELECT * FROM economic_events WHERE DATE(event_time) = '$todayDate' ORDER BY event_time ASC";
$res = $conn->query($sql);
...
<h1>📅 Today's Economic Events (<?= $todayDate ?>)</h1>
...
<div class="next-event-box">
    ...
    Time: <?= date('g:ia', strtotime($nextEvent['event_time'])) ?> IST<br><br>
...
<?php if(empty($events)): ?>
    <div class="no-events">No economic events scheduled for today.</div>
<?php else: ?>
    <table>
    <tr>
        <th>ID</th>
    ...
    <tr>
        ...
        <td><?= $event['impact'] ?></td>
        <td><?= date('g:ia', strtotime($event['event_time'])) ?></td>
        <td>
            <button class="btn-ai"
                onclick="askAI(
