...
elseif (strpos($text, '/analyze') === 0 || strpos($text, '/analyse') === 0) 
        {
            // Note: $text is already lowercased at the top of webhook.php
            // We uppercase the parts to easily match pairs and days like "EURJPY" and "FRI"
            $parts = explode(' ', strtoupper(trim($update_array['message']['text'])));
            
            if (count($parts) >= 2) 
            {
                $pair = $conn->real_escape_string(trim($parts[1]));
                ...
                if (isset($parts[2])) 
                {
                    ...
                    $dayShort = trim($parts[2]);
                    if (isset($dayMap[$dayShort])) 
                    {
                        // Convert UTC to IST to ensure accurate day filtering
                        $dayQuery = " AND DAYNAME(CONVERT_TZ(last_alert_time, '+00:00', '+05:30')) = '{$dayMap[$dayShort]}'";
                    }
                }

                // Fetch trade IDs matching the pair and day. 
                // Hard-capped at 50 to prevent GD memory crashes during PDF generation.
                $sql = "SELECT raw_trade_id FROM prediction_trade_data WHERE pair_name = '$pair' $dayQuery ORDER BY raw_trade_id DESC LIMIT 50";
                $res = $conn->query($sql);
                ...
                if ($res && $res->num_rows > 0) 
                {
                    ...
                    {
                        $tradeIds[] = $row['raw_trade_id'];
                    }
                }

                if (empty($tradeIds)) 
                {
                    sendMessage($chatId, "⚠️ No trades found for *$pair*" . (isset($dayShort) ? " on *$dayShort*" : "") . ".", true);
                } 
                else 
                {
                    ...
                    sendMessage($chatId, "⏳ Analyzing *$count* trades for *$pair*...\nGenerating charts via plotter. _You will receive the PDF report shortly._", true);

                    // Trigger plot/plotter.php in the background to handle the heavy rendering
                    $plotterPath = __DIR__ . '/plot/plotter.php';
                    $cmd = "php " . escapeshellarg($plotterPath) . " run_broadcast=1 trade_ids=" . escapeshellarg($idsString) . " > /dev/null 2>&1 &";
                    shell_exec($cmd);
                }
            } 
            else 
            {
                sendMessage($chatId, "❌ *Invalid format.*\nUsage: `/analyze EURJPY FRI`\nOr: `/analyze EURUSD`", true);
            }
        }
...
elseif ($text === '/status') 
{
    ...
    // 2. Month-wise Stats Query (Updated to use 'last_alert_time')
    $monthlySql = "
        SELECT 
            DATE_FORMAT(last_alert_time, '%M %Y') AS month_year,
            COUNT(*) as total_signals,
            SUM(CASE WHEN trade_result = 'win' THEN 1 ELSE 0 END) as total_wins,
            SUM(CASE WHEN trade_result = 'loss' THEN 1 ELSE 0 END) as total_losses
        FROM prediction_trade_data
        WHERE last_alert_time IS NOT NULL
        GROUP BY DATE_FORMAT(last_alert_time, '%Y-%m'), month_year
        ORDER BY DATE_FORMAT(last_alert_time, '%Y-%m') DESC
    ";
    ...
    $msg .= "Total: $totalSignals | W: $wins | L: $losses\n\n";

    $msg .= "📅 *MONTHLY BREAKDOWN*\n";
    
    if ($monthlyResult && $monthlyResult->num_rows > 0) {
        while($row = $monthlyResult->fetch_assoc()) {
            $mWins = $row['total_wins'] ?? 0;
            $mLosses = $row['total_losses'] ?? 0;
            $mValid = $mWins + $mLosses;
            
            // Calculate win rate for the specific month
            $mWinRate = ($mValid > 0) ? round(($mWins / $mValid) * 100, 2) : 0;
            $monthName = $row['month_year'];
            
            $msg .= "🔹 *$monthName*: $mWinRate% (W: $mWins | L: $mLosses)\n";
        }
    } else {
        $msg .= "No monthly data available yet.\n";
    }

    $msg .= "\n⏱ Checked at: " . date('d M Y H:i:s') . " IST";
  ...
  //Generate PDF of last N trade charts
        ...
        elseif (strpos($text, '/ss') === 0)
{
    $parts = explode(' ', trim($update_array['message']['text']));
    $ssUrl = trim($parts[1] ?? '');

    if (empty($ssUrl) || !filter_var($ssUrl, FILTER_VALIDATE_URL))
    {
        sendMessage($chatId, "❌ Usage: `/ss https://example.com`", true);
    }
    else
    {
        sendMessage($chatId, "📸 Taking screenshot of `$ssUrl`...", true);

        $pubKey    = 'ubx_WmDSoq6TIEBEi6hW';
        $secKey    = 'ubx_sk_1VFf9NmljfGclokxFpezTQBdDctGNQBT';
        ...
        $token   = hash_hmac('sha256', $params, $secKey);
        $api_url = "https://api.urlbox.io/v1/$pubKey/$token/png?$params";

        $img = @file_get_contents($api_url);

        if ($img && substr($img, 0, 8) === "\x89PNG\r\n\x1a\n")
        {
            $tmpFile = sys_get_temp_dir() . '/ss_' . uniqid() . '.png';
            file_put_contents($tmpFile, $img);
            sendPhoto($chatId, $tmpFile, "🖼 Screenshot of: $ssUrl");
            unlink($tmpFile);
        }
        else
        {
            sendMessage($chatId, "❌ Screenshot failed. Check the URL or API quota.", true);
        }
    }
}
