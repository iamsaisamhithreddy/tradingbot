...
if ($tradeResult && $tradeResult->num_rows > 0) {
    while ($row = $tradeResult->fetch_assoc()) {
        ...
        // last_alert_time: historical (<2025-06-01) stored as IST, live stored as UTC
        $alertStr = $row['last_alert_time'];
        $isHist = (substr($alertStr, 0, 10) < '2025-06-01');
        $alertTZ = $isHist ? $istTimezone : $utcTimezone;
        $istAlertTime = (new DateTime($alertStr, $alertTZ))->setTimezone($istTimezone)->format('d M H:i');
        ...
        echo "<div class='trade-item' id='trade-{$row['raw_trade_id']}' data-result='{$dataResult}'>
