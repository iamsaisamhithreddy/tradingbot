...
// Token access via GET parameter
...
if(!$access_granted && $provided_token !== ''){
    ...
    $found = false;
    if($stmt->fetch()){
        // Token is valid if expires_at is NULL (permanent) or in future
...
// Deny access if neither admin nor valid token
if(!$access_granted){
    header("HTTP/1.1 403 Forbidden");
    exit("Access denied.<br> ...TOKEN INVALID! GET TOKEN FROM ADMIN.... ");
}
