<?php
// =============================================================================
// TRADING CORE FUNCTIONS
// Shared logic for pattern evaluation and price target calculation.
// Extracted to ensure exact business rules are maintained across live and historical systems.
// =============================================================================
                ...
                if ($direction === 'UP') {
                    if ($c['close'] < $targetPrice) {
                        if (!$targetBroken) {
                            ...
                            if ($waveLength < 3 && !$isNewsBreak) return ['result' => 'setup_not_formed', 'reason' => 'Too fast.'];
                            $targetBroken = true;
                        }
                        if ($consecutiveRed >= 3) {
                            $c1 = $candles[$i + 1] ?? null;
                            ...
                            if ($c1['close'] > $c1['open']) return ['result' => 'win', 'reason' => 'Direct Win.', 'time' => $c1['time'], 'price' => $c1['close']];
                            if (!$c2) return ['result' => 'pending', 'reason' => 'Waiting for C2.'];
                            if ($c2['close'] > $c2['open']) return ['result' => 'win', 'reason' => 'MTG1 Win.', 'time' => $c2['time'], 'price' => $c2['close']];
